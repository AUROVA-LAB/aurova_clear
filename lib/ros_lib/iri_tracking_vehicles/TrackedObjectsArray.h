#ifndef _ROS_iri_tracking_vehicles_TrackedObjectsArray_h
#define _ROS_iri_tracking_vehicles_TrackedObjectsArray_h
...
#include "iri_tracking_vehicles/TrackedObject.h"
...
namespace iri_tracking_vehicles
{
  ...
  class TrackedObjectsArray : public ros::Msg
  {
    public:
      ...
      typedef iri_tracking_vehicles::TrackedObject _objects_type;
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      if(objects_lengthT > objects_length)
        this->objects = (iri_tracking_vehicles::TrackedObject*)realloc(this->objects, objects_lengthT * sizeof(iri_tracking_vehicles::TrackedObject));
      objects_length = objects_lengthT;
      for( uint32_t i = 0; i < objects_length; i++){
      offset += this->st_objects.deserialize(inbuffer + offset);
        memcpy( &(this->objects[i]), &(this->st_objects), sizeof(iri_tracking_vehicles::TrackedObject));
      }
     return offset;
    }

    const char * getType(){ return "iri_tracking_vehicles/TrackedObjectsArray"; };
    const char * getMD5(){ return "71e904774b1e4b990cdef40300152034"; };
