#ifndef _ROS_ublox_msgs_NavPVT_h
#define _ROS_ublox_msgs_NavPVT_h
  ...
  class NavPVT : public ros::Msg
  {
    ...
    virtual int serialize(unsigned char *outbuffer) const
    {
      ...
      *(outbuffer + offset + 3) = (u_headVeh.base >> (8 * 3)) & 0xFF;
      ...
      u_magDec.real = this->magDec;
      *(outbuffer + offset + 0) = (u_magDec.base >> (8 * 0)) & 0xFF;
      *(outbuffer + offset + 1) = (u_magDec.base >> (8 * 1)) & 0xFF;
      offset += sizeof(this->magDec);
      *(outbuffer + offset + 0) = (this->magAcc >> (8 * 0)) & 0xFF;
      *(outbuffer + offset + 1) = (this->magAcc >> (8 * 1)) & 0xFF;
      offset += sizeof(this->magAcc);
      return offset;
    }
    ...
    virtual int deserialize(unsigned char *inbuffer)
    {
      ...
      u_magDec.base = 0;
      u_magDec.base |= ((uint16_t) (*(inbuffer + offset + 0))) << (8 * 0);
      u_magDec.base |= ((uint16_t) (*(inbuffer + offset + 1))) << (8 * 1);
      this->magDec = u_magDec.real;
      offset += sizeof(this->magDec);
      this->magAcc =  ((uint16_t) (*(inbuffer + offset)));
      this->magAcc |= ((uint16_t) (*(inbuffer + offset + 1))) << (8 * 1);
      offset += sizeof(this->magAcc);
     return offset;
    }

    const char * getType(){ return "ublox_msgs/NavPVT"; };
    const char * getMD5(){ return "10f57b0db1fa3679c06567492fa4e5f2"; };
